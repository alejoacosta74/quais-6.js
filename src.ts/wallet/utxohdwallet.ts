...
export class UTXOHDWallet extends BaseWallet {
     ...
     */
    private deriveAddress(index: number, zone: string): AddressInfo{
    ...
    public async init(zone: string): Promise<void> {
        ...
        while (nakedCount < GAP) {
            ...
            if (!addressInfo) {
                throw new Error(`Failed to derive address with index ${derivationIndex} for zone: ${zone}`);
            }
